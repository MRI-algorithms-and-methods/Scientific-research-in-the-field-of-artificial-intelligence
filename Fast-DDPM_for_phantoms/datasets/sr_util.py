...
def masks_transform(mask, multichannel=True):
    totensor = torchvision.transforms.ToTensor()
    ...
    if multichannel:
        ...
        for i in range(mask.shape[2]):
            one_channel_mask = mask[:,:,i].copy()

            min_val = float(one_channel_mask.min())
            max_val = float(one_channel_mask.max())
            scale = 2 / (max_val - min_val)
            scale_l.append(scale)
            offset = -1 - min_val * scale
            offset_l.append(offset)
            one_channel_mask_rescaled  = one_channel_mask.astype(np.float32) * scale + offset
            list_of_masks.append(one_channel_mask_rescaled)
        arr_of_masks = np.array(list_of_masks).transpose(1, 2, 0)

        ref = totensor(arr_of_masks)
        scale = scale_l
        offset = offset_l
        ...
        scale = 2 / (max_val - min_val)
        offset = -1 - min_val * scale
        ref = mask.astype(np.float32) * scale + offset

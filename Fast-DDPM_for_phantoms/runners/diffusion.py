...
from datasets.phantoms import Phantoms
...
class Diffusion(object):
    ...
    def sg_train(self):
        ...
        elif self.args.dataset=='PHANTOMS':
            dataset = Phantoms(self.config.data.train_dataroot, self.config.data.image_size, split='train')
            print('Start training your Fast-DDPM model on PHANTOMS dataset.')
    ...
    def sg_sample_fid(self, model):
        ...
        if self.args.dataset=='LDFDCT':
            ...
            # BRATS for brain image translation
            sample_dataset = Phantoms(self.config.data.sample_dataroot, self.config.data.image_size, split='calculate')
            print('Start evaluate model on PHANTOMS dataset.')
        print('The inference sample type is {}. The scheduler sampling type is {}. The number of involved time steps is {} out of 1000.'.format(self.args.sample_type, self.args.scheduler_type, self.args.timesteps))
        ...
        with torch.no_grad():
            ...
            for batch_idx, sample in tqdm.tqdm(enumerate(sample_loader), desc="Generating image samples for FID evaluation."):
                ...
                offset = sample['offset_gt'] #.float().cpu().numpy()
                scale = sample['scale_gt'] #.float().cpu().numpy()

                #for i in range(n):
                    #x_gt[i, :, :] = (x_gt[i, :, :] - offset[i]) / scale[i]
                    #x[i, :, :] = (x[i, :, :] - offset[i]) / scale[i]

                for i in range(n):
                    for j in range(x_gt.shape[1]):
                        x_gt[i, j, :, :] = (x_gt[i, j, :, :] - offset[j][i].float().cpu().numpy()) / scale[j][i].float().cpu().numpy()
                        x[i, j, :, :] = (x[i, j, :, :] - offset[j][i].float().cpu().numpy()) / scale[j][i].float().cpu().numpy()
                        if j == 2:
                            x_gt[i, j, :, :] = x_gt[i, j, :, :] * 1e-3
                            x[i, j, :, :] = x[i, j, :, :] * 1e-3



                #x_gt = (x_gt - offset)/scale
                #x = (x - offset)/scale
                #x_gt = x_gt*255
                #x = x*255
                ...
                for i in range(x.shape[0]):
                    for j in range(x.shape[1]):
                        psnr_temp = calculate_psnr(x[i, j, :, :], x_gt[i, j, :, :])
                        ssim_temp = ssim(x_gt[i, j, :, :], x[i, j, :, :], data_range=2569)
                        PSNR += psnr_temp
                        SSIM += ssim_temp
                        psnr_list.append(psnr_temp)
                        ssim_list.append(ssim_temp)
                ...
                for i in range(0, n):
                    ...
                    #tvu.save_image(
                        #x_tensor[i], os.path.join(self.args.image_folder, "{}_{}_pt.png".format(self.ckpt_idx, img_id))
                    #)
                    np.save(os.path.join(self.args.image_folder, "{}_{}_pt_np.npy".format(case_name[i], self.ckpt_idx)),x[i])
                    #tvu.save_image(
                        #x_gt_tensor[i], os.path.join(self.args.image_folder, "{}_{}_gt.png".format(self.ckpt_idx, img_id))
                    #)
                    np.save(os.path.join(self.args.image_folder, "{}_{}_gt_np.npy".format(case_name[i], self.ckpt_idx)), x_gt[i])
                    img_id += 1
